...
from .api_utils import api_key, api_key2, client_id, client_secret
from spotipy.oauth2 import SpotifyClientCredentials, SpotifyOAuth

from .playlists import Track, Playlist

def get_user_location():
    print("Getting user location")
    # Geolocation API - gets user location through IP address. 
    url_2 = "https://api.getgeoapi.com/v2/ip/check?api_key={}".format(api_key2)
...
def get_user_weather(location):
    ...
    url = "https://api.openweathermap.org/data/2.5/weather?lat={}&lon={}&appid={}&units=metric".format(location[0], location[1], api_key)
...
def call_spotify(query):
    ...
    for item in results['tracks']['items']:
        if item['explicit'] == False:
            track = Track(item['name'], [artist['name'] for artist in item['artists']], item['id'], item['duration_ms'], item['external_urls']['spotify'])
